#!/usr/bin/env python
# -*- coding: utf-8 -*-
# from the DoorPi forum, saved for future
# usable and works
...
def get(**kwargs): return Nextion(**kwargs)
class Nextion(KeyboardAbstractBaseClass):
    name = 'Nextion Keyboard'
    ...
    def read_nextion(self):
        ...
        self._ser.write("page 0"+self._output_stop_flag)
        self._ser.flush()
        while not self._shutdown and self._ser.isOpen():
            # char aus buffer holen
            newChar = self._ser.read()
            if newChar is None or newChar == "": continue
            c = ord(newChar)
            if c==0xff and len(self._last_received_chars)==0: continue
            self._last_received_chars += str(newChar)
            logger.debug("new char %s -- %s -- read and is now %s", newChar, str(c), self._last_received_chars)
    ...
    def __init__(self, input_pins, output_pins, conf_pre, conf_post, keyboard_name, *args, **kwargs):
        ...
        self._thread = threading.Thread(target = self.read_nextion)
